...
public class GuideCustomView extends RelativeLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public void showOnce() {
        if (targetView != null) {
            mContent.getSharedPreferences("vipcare_guide", Context.MODE_PRIVATE).edit().putBoolean(generateUniqId(targetView), true).commit();
        }
    }
    ...
    public boolean hasShown() {
        if (targetView == null)
            return true;
        return mContent.getSharedPreferences("vipcare_guide", Context.MODE_PRIVATE).getBoolean(generateUniqId(targetView), false);
    }
    ...
    public void hide() {
        ...
        if (customGuideView != null) {
            ...
            this.removeAllViews();
            if (mDialog != null) {
                ((FrameLayout) mDialog.getWindow().getDecorView()).addView(this);
            } else {
            ...
            if (direction != null) {
                ...
                switch (direction) {
                    case TOP:
                        ...
                        guideViewParams.setMargins(offsetX, offsetY - height + top - (radius / 2), -offsetX, height - top - offsetY + (radius / 2));
                        break;
                    case LEFT:
                    ...
                    case LEFT_TOP:
                        ...
                        guideViewParams.setMargins(offsetX - width + left, offsetY - height + top, (width - left - offsetX) - 200, height - top - offsetY);
                        break;
                    case LEFT_BOTTOM:
                        this.setGravity(Gravity.RIGHT);
                    ...
                    case RIGHT_TOP:
                        ...
                        guideViewParams.setMargins((right + offsetX) / 4, offsetY - height + top + radius, -radius - offsetX, height - top - offsetY - radius);
                        break;
                    case RIGHT_BOTTOM:
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (mDialog != null) {
            if (backgroundColor != 0) {
                mDialog.getWindow().setBackgroundDrawable(new ColorDrawable(backgroundColor));
            } else {
                mDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.parseColor("#222222")));
            }
    ...
    public static class Builder {
        static GuideCustomView guiderView;
        ...
        public static Builder newInstance(Context ctx) {
            guiderView = new GuideCustomView(ctx);
            return instance;
        }

        public Builder setDialog(Dialog dialog) {
            guiderView.setDialog(dialog);
            return instance;
        }
